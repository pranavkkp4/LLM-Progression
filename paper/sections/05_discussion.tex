...
(88.8\%~$\to$~91.9\%) at higher cost~\cite{openai2026gpt56}.
...
must infer task type from unlabeled traffic. (iv) The v1 intervention was
selected on only 16 development items and evaluated once on 100 GSM8K items;
it covers one model family, one task, and one evaluation seed. Its four-point
primary gain is not statistically significant ($p=0.481$), and the Wilson
intervals remain broad. (v) Seeded sampling is backend-sensitive: CPU and CUDA
produced different sampled candidates, so exact replay requires the recorded
software and device context. (vi) The field moves weekly; public benchmark
figures are current as of August 10, 2026.
