...
Eight years separate BERT's 80.5 GLUE from Opus~5's reported 42/42 on the
2026 IMO problems. The
...
a fixed capability level is falling even faster, to the point where a \$1/M
...
decision that matters. A practical way to improve a deployed system today is
often to leave its weights fixed while measuring better ways to spend its
inference compute.
In our frozen GSM8K evaluation, four-sample plurality improved exact match
from 58\% to 62\%, while the any-sample oracle reached 79\%; that gap makes
selection, rather than candidate generation, the clearest next target. The
observed four-point primary gain remains uncertain ($p=0.481$).

\paragraph{Reproducibility.} All datasets (\texttt{code/data/}), analysis
scripts (\texttt{exp1}--\texttt{exp4}), the locked v1 runner and manifests,
complete live-experiment traces, unit tests (\texttt{tests/}), figure
generation, and this paper's build script are
