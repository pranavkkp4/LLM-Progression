...
July 2026, Anthropic released Claude Opus~5, which scored a perfect 42/42 on
the 2026 International Mathematical Olympiad without tools and quadrupled the
world record on ARC-AGI-3, a benchmark explicitly designed to measure reasoning
about novel problems~\cite{decoder2026opus5,arcprize2026opus5}. Between those
...
This paper has three goals. The first is \emph{descriptive}: to trace the
technical progression from BERT to the 2026 frontier --- GPT-5.6 (Sol, Terra,
Luna), Claude Fable~5 and Opus~5, Kimi~K3, and their contemporaries --- in one
place, with numbers. The second is \emph{quantitative}: to measure how fast
capability and cost-efficiency have actually improved, using public benchmark
and pricing series rather than impressions. The third is \emph{prescriptive}:
to test, with runnable code, three practical suggestions for getting more out
of existing models --- spending inference compute better, calibrating reasoning
effort, and routing tasks across models.
...
\begin{enumerate}
    ...
    \item A cost analysis showing a ${\sim}300\times$ decline in input-token
    ...
    \item A live, reproducible experiment showing self-consistency sampling
    materially improves a small open model's math accuracy over greedy
    decoding, plus analyses of reasoning-effort calibration
...
Two further shifts followed. First, \emph{test-time compute}: OpenAI's o1
(December 2024) showed that training models to produce long chains of thought
--- and spending more inference tokens --- improved math and coding
substantially; o1 reached 91.8\% on MMLU, effectively saturating
it~\cite{benchlm_mmlu}. Second, \emph{agency}: SWE-bench
Verified~\cite{jimenez2024swebench,openai2024swebenchverified} made
``resolve a real GitHub issue'' the standard hard task, and scores climbed from
49\% (Claude 3.5 Sonnet, October 2024) to 97\% (Claude Opus 5, July 2026) on
independent harnesses~\cite{vals2026swebench}. By mid-2026 the flagship
...
answering~\cite{openai2026gpt56,morphllm2026coding,tomshardware2026k3,coursiv2026opus5}.
...
the best proprietary systems~\cite{venturebeat2026k3,tomshardware2026k3}.

Table~\ref{tab:timeline} summarizes the arc; Figure~\ref{fig:mmlu} and
Figure~\ref{fig:swe} show the two benchmark series that bookend it.
